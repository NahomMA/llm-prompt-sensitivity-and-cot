...
For Problem~3 (prompt sensitivity), we perturbed one of the best-performing prompts from Problems~1--2 (e.g., the few-shot sentiment prompt) by:
\begin{itemize}
  \item Reordering the in-context examples.
  \item Rephrasing the task description (more formal vs. more conversational).
  \item Adding or removing an explicit constraint on output format.
\end{itemize}
The exact text of these variants follows the same structure as the base templates in Appendices~A and~B and can be reconstructed from the descriptions in the main report.
